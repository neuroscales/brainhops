...
class OmeZarrParser(FileParser):
    ...
    def from_file(cls, path: _tx.Union[str, PathLike]) -> _tx.Self:
        """Create an OmeZarrParser from a path to an OME-Zarr store."""
        cls._require_zarr()
    ...
    def from_bytes(cls, data: bytes) -> _tx.Self:
        """
        Create an OmeZarrParser from an OME-Zarr store made from byes
        """
        cls._require_zarr()
    ...
    def from_zarr(cls, source: _OmeZarrLike) -> _tx.Self:
        ...
        if isinstance(source, (str, PathLike)):
            return cls.from_zarr(zarr.open_group(str(source), mode="r"))
    ...
    def sniff(cls, other: _OmeZarrLike) -> bool:
        ...
        -- without fully loading it as an `OmeZarrParser`.
