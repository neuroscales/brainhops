...
from .modes import ModeLike, SimplifyLike, SimplifyPolicy, is_member
    ...
    def compute(
        ...
        *,
        ...
        simplify : simplify policy, default="analytic"
            ...
            single [`SimplifyPolicy`][brainhops.datamodel.enums.\
SimplifyPolicy] (`False`/`"none"`/`None`, `"analytic"`, `True`/`"numeric"`),
...
compute]: `t.simplify(policy, compute=mode)` is
        ...
        compute : [list of] name or type, optional
            The compose mode to gate on. The default, `False`, means no mode
            gating (every kind composed), i.e. `mode=None` in `compute`.
        """
        # `compute=False` is the sugar's "no mode gating" default; it maps to
        # `compute`'s `mode=None` (admit every kind). A real mode passes
        # straight through.
        return self.compute(
            None if compute is False else compute, simplify=policy
        )

    def _is_member(self, node: type, policy: SimplifyPolicy) -> bool:
        """Whether membership in the hierarchy set `node` is *established*
        from what may be inspected at `policy`, beyond the declared type.

        The base default establishes nothing; leaf and wrapper families
        override this. Declared membership (via `.register`) is answered by
        `is_member` before this hook is reached.
        """
        return False
    ...
    def is_member(
        self,
        kind: tx.Any,
        policy: tx.Union[SimplifyPolicy, bool, str, None] = "analytic",
    ) -> bool:
        """Whether this transformation is a member of `kind` at `policy`.

        Convenience wrapper over the module-level
        [`is_member`][brainhops.datamodel.transformations.is_member].
        """
        return is_member(self, kind, policy)
