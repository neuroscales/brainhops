...
mov = src(
    dst.geometry
).reslice()  # -> Image(data=...).geometry == CartesianField(dst.shape)
mov = src(
    wrp
).reslice()  # -> Image(data=...).geometry == CartesianField(ras_coords.shape)
mov = src(
    disp
).reslice()  # -> Image(data=...).geometry == CartesianField(vox_disp.shape)
mov = src(
    ras2ras
).reslice()  # -> raise Exception("Cannot guess output geometry")
```
...
mov = src(dsp).reslice(
    dsp.geometry
)  # -> Image(data=...).geometry = dsp.geometry
```
