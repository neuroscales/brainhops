...
def make_affine(cs_in, cs_out, matrix=None):
    ni = len(cs_in.axes)
    no = len(cs_out.axes)
    if matrix is None:
...
def make_linear(cs_in, cs_out, matrix=None):
    ni = len(cs_in.axes)
    no = len(cs_out.axes)
    if matrix is None:
        matrix = np.eye(no, ni)
...
def make_field(cs_in, cs_out, shape):
...
class TestExpandDimsAffine:
    ...
    def test_expand_output_adds_zero_rows(self, xy, axes):
        t = make_affine(xy, xy, matrix=np.array([
            [2.0, 0.0, 10.0],
            [0.0, 3.0, 20.0],
        ]))
    ...
    def test_expand_input_adds_zero_columns(self, xy, axes):
        t = make_affine(xy, xy, matrix=np.array([
            [2.0, 0.0, 10.0],
            [0.0, 3.0, 20.0],
        ]))
...
class TestExpandDimsSequence:
    ...
    def test_expand_input_only_touches_first_transform(self, xy, axes):
    ...
    def test_expand_output_only_touches_last_transform(self, xy, axes):
        first = make_affine(xy, xy)
        second = make_affine(xy, xy)
        seq = Sequence(transformations=[first, second], input=xy, output=xy)
...
class TestMakeSameAxes:
    ...
    def test_expands_missing_axis(self, xy, xyz, axes):
...
class TestAdapt:
    ...
    def test_matching_axes_returns_identity(self, xyz):
        later = make_affine(xyz, xyz)
        earlier = make_affine(xyz, xyz)
        adapted = _adapt(later, earlier)
        assert isinstance(adapted, Identity)

    def test_reordered_axes_returns_permutation(self, xyz, zyx):
    ...
    def test_permutation_actually_reorders_correctly(self, xyz, zyx):
    ...
    def test_dimensionality_mismatch_raises(self, xy, xyz):
    ...
    def test_no_matching_axis_raises_value_error(self, axes):
...
class TestSequenceComputeWithAdaptAndExpand:
    ...
    def test_reordered_axes_compose_through_sequence(self, xyz, zyx):
