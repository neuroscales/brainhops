...
def test_affine_with_coordinates():
    ...
    sequence = Sequence(transformations=[field, affine])
    assert isinstance(sequence.transformations[0].field, da.Array)
...
def test_linear_with_coordinates():
    ...
    linear = Linear(matrix=np.asarray(
        [[2, 1, 0], [1, 2, 1], [0, 0, 1]]))
    sequence = Sequence(transformations=[field, linear])
    output = sequence.compute()
...
def test_displacement_with_coordinates():
    ...
    sequence = Sequence(transformations=[field, displacement])
    output = sequence.compute()
    for x in range(5):
        for y in range(5):
            for z in range(5):
...
def test_affine_displacement_coordinates():
    ...
    sequence = Sequence(
        transformations=[field, displacement, affine, input_coord])
    output = sequence.compute()
    for x in range(10):
        for y in range(10):
            for z in range(10):
...
def test_permutation_with_field():
    ...
    sequence = Sequence(transformations=[field, permutation])
    output = sequence.compute()
    for x in range(5):
        for y in range(5):
            for z in range(5):
...
def test_scale_with_coordinates():
    ...
    sequence = Sequence(transformations=[field, scale])
    output = sequence.compute()
    for x in range(5):
        for y in range(5):
            for z in range(5):
...
def test_translation_with_coordinates():
    ...
    for i in range(len(g_vals)):
        identity[:, :, :, i] = g_vals[i]
    field = CoordinatesField(field=identity)
    translation = Translation(translation=[1, 2, 0])
    sequence = Sequence(transformations=[field, translation])
    output = sequence.compute()
    for x in range(5):
        for y in range(5):
            for z in range(5):
