...
def test_key_and_list_restrict_to_those() -> None:
    ...
    # means the set.
    assert _lower_simplify(Affine) == {None: none, (Affine, None): analytic}
...
def test_mapping_fallback_and_collisions() -> None:
    ...
    t = _lower_simplify({"affine": "numeric", Affine: "none"})
...
def test_special_and_symbol_keys() -> None:
    ...
    assert _lower_key(Rotation) == (Rotation, None)
    assert _lower_key(Identity) == (Identity, None)
...
def test_invalid_keys_raise() -> None:
    ...
    assert _lower_key(Sequence) == (Sequence, None)
    assert _lower_key(MultiscaleField) == (MultiscaleField, None)
